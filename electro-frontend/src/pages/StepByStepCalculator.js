    ...
    try {
      ...
      for (let index = 0; index < selectedRooms.length; index++) {
        ...
        const roomData = {
          ...
          maxSwitches: roomConfig.maxSwitches ?? null,
          maxDoors: roomConfig.maxDoors ?? null,
          maxWindows: roomConfig.maxWindows ?? null,
          maxLights: roomConfig.maxLights ?? null,
        };
                ...
                <div key={room.id} className="room-config-panel">
                  ...
                  {isExpanded && (
                    <div className="room-config-content">
                      ...
                      <div className="form-group">
                        <label style={{ fontWeight: 600, marginBottom: '0.5rem', display: 'block' }}>Лимиты объектов в комнате</label>
                        <div className="field-hint">
                          Ограничьте максимальное количество объектов для 3D-редактора. Оставьте пустым — без ограничений.
                        </div>
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem', marginTop: '0.5rem' }}>
                          {[
                            { label: 'Макс. выключателей', field: 'maxSwitches' },
                            { label: 'Макс. дверей', field: 'maxDoors' },
                            { label: 'Макс. окон', field: 'maxWindows' },
                            { label: 'Макс. ламп', field: 'maxLights' },
                          ].map(({ label, field }) => (
                            <div key={field} className="editor-field" style={{ marginBottom: 0 }}>
